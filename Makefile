...
clean:
	rm -f fade *.o fade.hex

fade.o: fade.c
	 $(CPP) -Os -mmcu=$(MCU) -c -o $@ $^ $(INCLUDE) $(DHT_INCLUDES)

fade: fade.o arduino_core_c arduino_core_cpp
	$(CC) -Os -mmcu=$(MCU) *.o -o $@

fade.hex: fade
	avr-objcopy -O ihex -R .eeprom fade fade.hex

install: fade.hex
	# device can also be bound to /dev/ttyACM0 YMMV.
	avrdude -F -V -c arduino -p ATMEGA328P -P /dev/ttyUSB0 -b 115200 -U flash:w:fade.hex
