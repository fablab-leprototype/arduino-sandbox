...
clean:
	rm -f sketch sketch.o sketch.hex *.o

sketch.o: sketch.c
	 $(CC) -Os -mmcu=$(MCU) -c -o sketch.o sketch.c $(INCLUDE)

sketch: sketch.o arduino_core_c arduino_core_cpp
	$(CC) -mmcu=$(MCU) *.o -o sketch

sketch.hex: sketch
	avr-objcopy -O ihex -R .eeprom sketch sketch.hex

install: sketch.hex
	# device can also be bound to /dev/ttyACM0 YMMV.
	avrdude -F -V -c arduino -p ATMEGA328P -P /dev/ttyUSB0 -b 115200 -U flash:w:sketch.hex

# arduino specific core lib
